...
const className = 'PascaS2AdminUserManagemenController';
const renderName = 'pasca/s2';
const routeName = 'pasca.s2';
class PascaS2AdminUserManagemenController {
    ...
    async insert_responden({ request, response, session }) {
        try {
            ...
            let cek_nim = await PascaS2Services_1.default.get_validasi_nim(nim);
            if (cek_nim) {
                const hash_password = await Hash_1.default.make(nim);
                const password_clear = nim;
                await PascaS2Services_1.default.create_responden(nim, cek_nim.nama_mhs, cek_nim.periode, password_clear, hash_password);
    ...
    async ajax_get_responden({ request }) {
        try {
            const { nim } = request.all();
            const responden = await PascaS2Services_1.default.get_responden(nim);
            return { responden };
        }
        catch (error) {
            console.log(error);
...
//# sourceMappingURL=PascaS2AdminUserManagemenController.js.map
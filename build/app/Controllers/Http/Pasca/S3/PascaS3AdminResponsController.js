...
class PascaS3AdminResponsController {
    async pengisi({ view, auth }) {
        ...
        const daftar_sasaran = await PascaS3Services_1.default.get_list_sasaran();
    ...
    async ajax_data_pengisi({ request }) {
        try {
            ...
            let { kd_fjjp7_non, kd_fjjp7_reg } = await PascaS3Services_1.default.get_users_mapping_kd_fjjp7(kd_fjjp7);
            if (periode_wisuda !== 'null') {
                console.log(periode_wisuda);
                const get_data_pengisi = await PascaS3Services_1.default.get_data_pengisi(periode_wisuda, kd_fjjp7_non, kd_fjjp7_reg);
                return { get_data_pengisi };
            }
            else {
                const get_periode_wisuda = await PascaS3Services_1.default.get_sasaran();
                const get_data_pengisi = await PascaS3Services_1.default.get_data_pengisi(get_periode_wisuda.tahun, kd_fjjp7_non, kd_fjjp7_reg);
                return { get_data_pengisi };
            }
        }
        catch (error) {
    ...
    async export_hasil_users({ request }) {
        try {
            ...
            for (let index = 0; index < table_name.length; index++) {
                const get_jawabans = await PascaS3Services_1.default.get_jawaban_users(periode_wisuda, kd_fjjp7_non, kd_fjjp7_reg, table_name[index]);

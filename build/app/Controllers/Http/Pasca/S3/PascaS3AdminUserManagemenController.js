...
const className = 'PascaS3AdminUserManagemenController';
const renderName = 'pasca/s3';
const routeName = 'pasca.s3';
class PascaS3AdminUserManagemenController {
    ...
    async insert_responden({ request, response, session }) {
        try {
            ...
            let cek_nim = await PascaS3Services_1.default.get_validasi_nim(nim);
            if (cek_nim) {
                const hash_password = await Hash_1.default.make(nim);
                const password_clear = nim;
                await PascaS3Services_1.default.create_responden(nim, cek_nim.nama_mhs, cek_nim.periode, password_clear, hash_password);
    ...
    async ajax_get_responden({ request }) {
        try {
            const { nim } = request.all();
            const responden = await PascaS3Services_1.default.get_responden(nim);
            return { responden };
        }
        catch (error) {
            console.log(error);
    ...
    async edit_responden({ request, response, session }) {
        try {
            ...
            const cek_email = await PascaS3Services_1.default.get_availabe_email(email);
            if (cek_email) {
                Global_1.message(session, 'notification', 'danger', 'email sudah pernah digunakan!');
                return response.redirect('back');
            }
            const edit = await PascaS3Services_1.default.edit_responden(nim, email);
            if (edit) {
...
exports.default = PascaS3AdminUserManagemenController;
//# sourceMappingURL=PascaS3AdminUserManagemenController.js.map
...
class D3AdminsController {
    ...
    async ajax_data_index({ request, session, response }) {
        try {
            ...
            let get_list_kd_fjjp7 = await D3Services_1.default.get_list_kdfjjp7();
            console.log(get_list_kd_fjjp7);
            let arr_kd_fjjp7 = [];
            for (let i = 0; i < get_list_kd_fjjp7.length; i++) {
                arr_kd_fjjp7.push(get_list_kd_fjjp7[i].kd_fjjp7);
            }
            const get_data = await D3Services_1.default.get_data_index(tahun, periode, arr_kd_fjjp7);
    ...
    async cek_populasi({ request, session, response }) {
        try {
            ...
            let get_populasi = await D3Services_1.default.get_populasi(tahun);
            return { get_populasi };
        }
        catch (error) {
            console.log(error);
    ...
    async insert_populasi({ request, session, response }) {
        try {
            ...
            if (alumni) {
                ...
                const status = await D3Services_1.default.insert_populasi(populasi);
                if (status) {
    ...
    async set_sasaran({ request, session }) {
        try {
            ...
            let current_sasaran = await D3Services_1.default.get_sasaran();
            if (current_sasaran) {
                let num_current = Number(current_sasaran.tahun);
                let num_new_sasaran = Number(tahun_periode);
                let cek_sasaran = await D3Services_1.default.cek_sasaran(tahun_periode);
                if (!cek_sasaran && num_new_sasaran > num_current) {
                    let status_import = 0;
                    const get_status_monitoring = await D3Services_1.default.get_status_monitoring(tahun);
                    if (get_status_monitoring) {
                        status_import = 1;
                    }
                    let update = await D3Services_1.default.set_sasaran(tahun_periode, status_import);
                    if (update) {
    ...
    async jadwal({ view, auth }) {
        await auth.authenticate();
        const tahunSasaran = await D3Services_1.default.get_sasaran();
        const get_jadwal = await D3Services_1.default.get_jadwal();
    ...
    async set_jadwal({ request, session, response, auth }) {
        ...
        try {
            ...
            const update_jadwal = await D3Services_1.default.set_jadwal(begin, end);
            if (update_jadwal) {
...
exports.default = D3AdminsController;
//# sourceMappingURL=D3AdminsController.js.map
...
//TODO: hubungkan routes dengan view
  ...
  public async register_users({request,session, response}){
      try {
          let {username, nama, email, password, permission_d3, permission_pasca_s2, permission_pasca_s3, permission_profesi} = request.all()

          const hash_password = await Hash.make(password)
          //insert users
          const insert_users = await User.insert_users(username, nama, email, hash_password, permission_d3, permission_pasca_s2, permission_pasca_s3, permission_profesi)
          if(insert_users){
            // kirim email
            const emailData = {
                nama: nama,
  ...
  public async update_users({request, session, response}){
      try {
          const {username, nama, permission_d3, permission_pasca_s2, permission_pasca_s3, permission_profesi} = request.all()
          const update_akun = await User.update_users(username, nama, permission_d3, permission_pasca_s2, permission_pasca_s3, permission_profesi)
          if(update_akun){

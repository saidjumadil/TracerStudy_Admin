...
export default class D3AdminsController {
  ...
  public async sasaran({ view }) {
    const get_sasaran = await Services.get_sasaran()
    return view.render('d3/sasaran', {get_sasaran})
  }
  ...
  public async set_sasaran({request, response, session}){
    try {
      ...
      var tahun_periode = tahun.concat(periode);
      ...
      message(
        ...
        'notification_sasaran',
        'danger',
        'Gagal mengubah sasaran Tracer Study'
      )
      return response.redirect('back')
  ...
  public async jadwal({ view }) {
    const get_jadwal = await Services.get_jadwal()
    return view.render('d3/jadwal', {get_jadwal})
  }
    ...
    try {
      ...
      if(update_jadwal){
        message(
          ...
          'notification_jadwal',
          'success',
          'Berhasil mengubah jadwal Tracer Study'
        )
        return response.redirect('back')
      }
    } catch (error) {
      ...
      message(
        ...
        'notification_jadwal',
        'danger',
        'Gagal mengubah jadwal Tracer Study'
      )
      return response.redirect('back')
    }
  }
